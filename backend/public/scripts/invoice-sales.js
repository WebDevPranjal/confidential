...
function showBatchDetails(batches) {
  ...
  tbody.innerHTML = ''; // Clear the existing content in tbody
  ...
  if (selectedProduct) {
      ...
      selectedProduct.batches.forEach(batch => {
          if (batch) {
              ...
              appendCell(tableRow, batch.batch);
              appendCell(tableRow, batch.quantity);
              appendCell(tableRow, new Date(batch.expireDate).toISOString().split('T')[0]);
              appendCell(tableRow, batch.packaging);
              appendCell(tableRow, batch.mrp);

              tbody.appendChild(tableRow);

              tableRow.addEventListener('click', function() {
                  document.getElementById('selected-batch').value = batch.batch;
...
function addToTable() {
    ...
    const productName = document.getElementById('product').value;
    const selectedBatch = document.getElementById('selected-batch').value;
    ...
    const values = [productName, selectedBatch, expireDate, mrp, salerate, quantity,free, discount];
    ...
    const elementIds = ['product', 'selected-batch', 'product-date', 'product-mrp', 'salerate', 'quantity', 'free', 'discount'];
...
function tableData() {
    ...
    tableRows.forEach(row => {
      const rowData = {
        ...
        discount: row.cells[7].innerText,
        free: row.cells[6].innerText,
      };
